...
from fastchat.model import get_conversation_template #TODO: look into conversation templates
...
def get_embedding_matrix(model):
    if isinstance(model, GPTJForCausalLM) or isinstance(model, GPT2LMHeadModel):
        ...
        if isinstance(model.model, nn.DataParallel):
            return model.model.module.embed_tokens.weight
        else:
...
def get_embeddings(model, input_ids):
    if isinstance(model, GPTJForCausalLM) or isinstance(model, GPT2LMHeadModel):
        ...
        if isinstance(model.model, nn.DataParallel):
            return model.model.module.embed_tokens(input_ids)
        else:
...
class AttackPrompt(object):
    ...
    def __init__(self,
        ...
        self.def_control = def_control_init
    ...
    # _update_ids 方法：根据不同的对话模板，该方法更新实例的一些内部变量，包括角色的切片（slices）和输入的 Tensor（input_ids）。这个方法是在初始化的时候调用的。
    def _update_ids(self):
        if self.def_control:
            self.conv_template.append_message(self.conv_template.roles[0],
                f"{self.def_control} {self.goal} {self.control}")
        else:
            self.conv_template.append_message(self.conv_template.roles[0],
                f"{self.goal} {self.control}")
        ...
        if self.conv_template.name=='llama-2':
            ...
            self.conv_template.update_last_message(f". {self.def_control}")
            toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
            self._defense_slice = slice(self._user_role_slice.stop, max(self._user_role_slice.stop, len(toks)))
            
            separator = ' ' if self.def_control else ''
            self.conv_template.update_last_message(f". {self.def_control}{separator}{self.goal}")
            toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
            self._goal_slice = slice(self._defense_slice.stop,
                max(self._defense_slice.stop, len(toks))) 
            
            separator2 = ' ' if self.goal else ''
            self.conv_template.update_last_message(f". {self.def_control}{separator}{self.goal}{separator2}{self.control}")
            toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
            ...
            if python_tokenizer:
                ...
                self.conv_template.update_last_message(
                    f"{self.def_control}")
                toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
                self._defense_slice = slice(self._user_role_slice.stop, max(self._user_role_slice.stop, len(toks) - 1))
                
                separator = ' ' if self.def_control else ''
                self.conv_template.update_last_message(f"{self.def_control}{separator}{self.goal}")
                toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
                self._goal_slice = slice(self._defense_slice.stop,
                    max(self._defense_slice.stop, len(toks) - 1)) 
                
                separator2 = ' ' if self.goal else ''
                self.conv_template.update_last_message(
                    f"{self.def_control}{separator}{self.goal}{separator2}{self.control}") 
                toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
    ...
    @torch.no_grad()
    def def_logits(self, model, test_controls=None, return_ids=False):
        pad_tok = -1
        if test_controls is None:
            test_controls = self.def_control_toks
        if isinstance(test_controls, torch.Tensor):
            if len(test_controls.shape)==1:
                test_controls = test_controls.unsqueeze(0)
            test_ids = test_controls.to(model.device)
            ...
            max_len = self._defense_slice.stop - self._defense_slice.start
            test_ids = [
                torch.tensor(self.tokenizer(control, add_special_tokens=False).input_ids[:max_len], device=model.device)
                for control in test_controls
            ]
        ...
        if not (test_ids[0].shape[0]==self._defense_slice.stop - self._defense_slice.start):
            raise ValueError((
                f"test_controls must have shape "
                f"(n, {self._defense_slice.stop - self._defense_slice.start}), "
                f"got {test_ids.shape}"
            ))
        
        # print(test_ids.shape)   (512, 20)
        locs = torch.arange(self._defense_slice.start, self._defense_slice.stop).repeat(test_ids.shape[0], 1).to(
            model.device)
        ids = torch.scatter(
            self.input_ids.unsqueeze(0).repeat(test_ids.shape[0], 1).to(model.device),
            1,
    ...
    def def_control_loss(self, logits, ids):
        crit = nn.CrossEntropyLoss(reduction='none')
        loss_slice = slice(self._defense_slice.start - 1, self._defense_slice.stop - 1)
        loss = crit(logits[:, loss_slice, :].transpose(1, 2), ids[:, self._defense_slice])
        return loss
    ...
    @property
    def def_control_str(self):
        return self.tokenizer.decode(self.input_ids[self._defense_slice]).strip()
    
    @def_control_str.setter
    def def_control_str(self, control):
        self.def_control = control
        self._update_ids()
    
    @property
    def def_control_toks(self):
        return self.input_ids[self._defense_slice]
    
    @def_control_toks.setter
    def def_control_toks(self, control_toks):
        self.def_control = self.tokenizer.decode(control_toks)
        self._update_ids()
    
    @property
    def prompt(self):
        # return self.tokenizer.decode(self.input_ids[self._goal_slice.start:self._control_slice.stop])
        return self.tokenizer.decode(self.input_ids[self._goal_slice.start:self._defense_slice.stop])
...
class PromptManager(object):
    ...
    def __init__(self,
        ...
        self._refuse_prompts = [
            ...
            for goal, refuse_target in zip(goals, refuse_targets)
        ]

        if benign_file:
            benign_data = pd.read_csv(benign_file)
            benign_goals, benign_targets = benign_data["query"], benign_data["answer"]

            self._benign_prompts = [
                managers['AP'](  # AttackPrompt
                    goal,
                    target,
                    tokenizer,
                ...
                for goal, target in zip(benign_goals, benign_targets)
            ]
            self.benign_idx = np.random.permutation(len(self._benign_prompts))
    ...
    def refuse_generate(self, model, gen_config=None):
        if gen_config is None:
            gen_config = model.generation_config
            gen_config.max_new_tokens = 16
        
        return [prompt.generate(model, gen_config) for prompt in self._refuse_prompts]
    ...
    def refuse_generate_str(self, model, gen_config=None):
        return [
            self.tokenizer.decode(output_toks)
            for output_toks in self.refuse_generate(model, gen_config)
        ]
    ...
    def def_grad(self, model):
        return sum([prompt.def_grad(model) for prompt in self._refuse_prompts])
    ...
    def get_refuse_prompt(self, i):
        return self._refuse_prompts[i]

    def update_benign_idx(self):
        self.benign_idx = np.random.permutation(len(self._benign_prompts))

    def get_benign_prompt(self, i):
        return self._benign_prompts[self.benign_idx[i]]
    
    def def_logits(self, model, test_controls=None, return_ids=False):
        vals = [prompt.def_logits(model, test_controls, return_ids) for prompt in self._prompts]
        if return_ids:
            return [val[0] for val in vals], [val[1] for val in vals]
        else:
            return vals
    ...
    def def_control_loss(self, logits, ids):
        return torch.cat(
            [
                prompt.def_control_loss(logit, id).mean(dim=1).unsqueeze(1)
                for prompt, logit, id in zip(self._prompts, logits, ids)
            ],
            dim=1
        ).mean(dim=1)
    ...
    def control_str(self, control):
        ...
        for prompt in self._refuse_prompts:
            prompt.control_str = control
    ...
    def control_toks(self, control_toks):
        ...
        for prompt in self._refuse_prompts:
            prompt.control_str = control_toks
    
    @property
    def def_control_str(self):
        return self._prompts[0].def_control_str
    
    @property
    def def_control_toks(self):
        return self._prompts[0].def_control_toks
    
    @def_control_str.setter
    def def_control_str(self, control):
        for prompt in self._prompts:
            prompt.def_control_str = control
        for prompt in self._refuse_prompts:
            prompt.def_control_str = control
    ...
    def def_control_toks(self, control_toks):
        for prompt in self._prompts:
            prompt.def_control_toks = control_toks
        for prompt in self._refuse_prompts:
            prompt.def_control_toks = control_toks
...
class MultiPromptAttack(object):
    ...
    @property
    def def_control_str(self):
        return self.prompts[0].def_control_str
    
    @def_control_str.setter
    def def_control_str(self, def_control):
        for prompts in self.prompts:
            prompts.def_control_str = def_control
    
    @property
    def def_control_toks(self):
        return [prompts.def_control_toks for prompts in self.prompts]
    
    @def_control_toks.setter
    def def_control_toks(self, control):
        if len(control)!=len(self.prompts):
            raise ValueError("Must provide control tokens for each tokenizer")
        for i in range(len(control)):
            self.prompts[i].def_control_toks = control[i]
    ...
    def defense_step(self, *args, **kwargs):
        
        raise NotImplementedError("Defense step function not yet implemented")
    ...
    def run(self,
            ...
            control_weight=None,
            benign_weight=None,
            refuse_target_weight=None,
            anneal=True,
            anneal_from=0,
            prev_loss=np.infty,
            prev_def_loss=np.infty,
        ...
        steps = 0
        loss = best_loss = 1e6
        def_loss = best_def_loss = 1e6
        best_control = self.control_str
        best_def_control = self.def_control_str
        runtime = 0.
        ...
        for i in range(n_steps):
            ...
            if i % defense_freq==0 and run_defense:
                ...
                if run_defense:
                    ...
                    keep_control = True if not anneal else P(prev_def_loss, def_loss, i + anneal_from)
                if keep_control:
                    self.def_control_str = def_control
                
                prev_def_loss = def_loss
                if def_loss < best_def_loss:
                    best_def_loss = def_loss
                    best_def_control = def_control
                print('Current Defense Loss:', def_loss, 'Best Defense Loss:', best_def_loss)
            
            iters.append(i)
            rec_losses.append(loss)
            rec_def_losses.append(def_loss)
            ...
            if self.logfile is not None and (i + 1 + anneal_from) % test_steps==0:
                if not run_defense:
                    ...
                    last_def_control = self.def_control_str
                    self.control_str = best_control
                    self.def_control_str = best_def_control
                
                model_tests = self.test_all()
                self.log(i + 1 + anneal_from, n_steps + anneal_from, self.control_str, self.def_control_str, loss,
                    def_loss, runtime, model_tests, verbose=verbose)
                
                if not run_defense:
                    self.control_str = last_control
                    self.def_control_str = last_def_control
        
        return self.control_str, loss, self.def_control_str, def_loss, steps
    ...
    def test_all(self):
        ...
        all_prompts = [
            self.managers['PM'](  # PromptManager
                ...
                [""] * len(self.goals + self.test_goals),
                worker.tokenizer,
                worker.conv_template,
                self.control_str,
                self.def_control_str,
    ...
    def log(self, step_num, n_steps, control, def_control, loss, def_loss, runtime, model_tests, verbose=True):
        ...
        log['def_controls'].append(def_control)
        log['losses'].append(loss)
        log['def_losses'].append(def_loss)
...
class ProgressiveMultiPromptAttack(object):
    ...
    def run(self,
        ...
        if self.logfile is not None:
            ...
            log['params']['refuse_target_weight'] = refuse_target_weight
        ...
        while step < n_steps:
            print(f"num_goals: {num_goals}, attack steps: {n_steps - step}")
            attack = self.managers['MPA']( # MultiPromptAttack
                self.goals[:num_goals],
                self.targets[:num_goals],
                self.refuse_targets[:num_goals],
                self.workers[:num_workers],
                self.control,
                self.def_control[0] if isinstance(self.def_control, list) else self.def_control,
                self.test_prefixes,
                self.logfile,
                self.managers,
                self.test_goals,
            ...
            if num_goals < len(self.goals):
                ...
                if num_workers < len(self.workers):
                    ...
                    attack.log(step, n_steps, self.control, self.def_control, loss, def_loss, 0., model_tests, verbose=verbose)
                    break
                else:
        ...
        return self.control, self.def_control, step
...
class IndividualPromptAttack(object):
    ...
    def __init__(self,
        ...
        control_init : str, optional
            A string used to control the attack (default is "! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! !")
        def_control_init : str, optional
            A string used to defend the attack
        test_prefixes : list, optional
            A list of prefixes to test the attack (default is ["I'm sorry", "I apologize", "As an", "I'm just", "I'm an", "I cannot", "Hello!"])
        logfile : str, optional
            A file to which logs will be written
        ...
        self.def_control = def_control_init
        self.def_control_init = def_control_init
        self.test_prefixes = test_prefixes
        self.logfile = logfile
        self.managers = managers
    ...
    def run(self,
            ...
            benign_weight: Optional[Any] = None,
            refuse_target_weight: Optional[Any] = None,
            anneal: bool = True,
            test_steps: int = 50,
            incr_control: bool = True,
            stop_on_success: bool = True,
        ...
        for i in range(len(self.goals)):
            ...
            attack = self.managers['MPA'](  # MultiPromptAttack
                self.goals[i:i + 1],
                self.targets[i:i + 1],
                self.refuse_targets[i:i + 1],
                self.workers,
                self.control,
                self.def_control[i] if isinstance(self.def_control, list) else self.def_control,
                self.test_prefixes,
                self.logfile,
                self.managers,
                self.test_goals,
        ...
        return self.control, self.def_control, n_steps
...
class EvaluateAttack(object):
    ...
    @torch.no_grad()
    def run(self, steps, controls, def_controls, batch_size, max_new_len=60, verbose=True):
        ...
        prev_control = 'haha'
        for step, (control, def_control) in enumerate(zip(controls, def_controls)):
    ...
    def run(model, tasks, results):
        while True:
            ...
            if fn=="grad":
                ...
                with torch.enable_grad():
                    results.put(ob.def_grad(*args, **kwargs))
            else:
                with torch.no_grad():
                    if fn=="logits":
                        results.put(ob.logits(*args, **kwargs))
                    elif fn=="def_logits":
                        results.put(ob.def_logits(*args, **kwargs))
...
def get_refuse_targets(params):
    train_refuse_targets = getattr(params, 'goals', [])
    offset = getattr(params, 'data_offset', 0)
    model = getattr(params, 'model_paths')[0].split("/")[-1].split("-")[0]
    
    if params.train_data:
        base_name, extension = params.train_data.rsplit('.', 1)
        new_filename = f"{base_name}_refuse_{model}.{extension}"
        train_data = pd.read_csv(new_filename)
        train_refuse_targets = train_data['target'].tolist()[offset:offset + params.n_train_data]
    
    return train_refuse_targets
